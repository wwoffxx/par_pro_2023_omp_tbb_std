...
#include <omp.h>
#include <random>
#include "../../../modules/task_2/pinezhanin_e_monte_carlo/monte_carlo.h"
...
#pragma omp parallel for schedule(static, chunk) num_threads(omp_get_num_procs()) reduction(+: result)
    for (int i = 0; i < number_points; i++) {
        for (int j = 0; j < dimension; j++) {
            point[j] = uniform_distribution[j](gen);
        }
