...
TEST(Parallel_Operations_STD, Test_Image_1) {
  const Image example({255});
  Image seq_image(example), par_image(example);
  linearHistogramStretching(seq_image.data(), seq_image.size());
  linearHistogramStretchingParallel(par_image.data(), par_image.size());
  ASSERT_EQ(seq_image, par_image);
}

TEST(Parallel_Operations_STD, Test_Image_4) {
...
TEST(Parallel_Operations_STD, Test_Image_9) {
...
TEST(Parallel_Operations_STD, Test_Image_16) {
...
TEST(Parallel_Operations_STD, Test_Image_25) {
