...
#include "../../../modules/task_4/kudryavtsev_n_gor_gauss/gor_gauss.h"

TEST(STD_parallel, No_throw_when_empty_image) {
  const int countRows = 0;
  const int countColumns = 0;
  const int countThreads = 4;
  auto image = genImage(countRows, countColumns);
  ASSERT_NO_THROW(gaussianFilterSTDParallel(image, countRows, countColumns, countThreads));
}

TEST(STD_parallel, Invalid_count_rows_or_columns) {
  const int countRows = 5;
  const int countColumns = 5;
  const int countThreads = 4;
  vector<intensityType> image = { 54, 251, 169, 80, 159, 95, 251, 220, 90 };
  ASSERT_ANY_THROW(gaussianFilterSTDParallel(image, countRows, countColumns, countThreads));
}

TEST(STD_parallel, Works_with_3x3_image) {
  const int countRows = 3;
  const int countColumns = 3;
  const int countThreads = 4;
  vector<intensityType> image = { 54, 251, 169, 80, 159, 95, 251, 220, 90 };
  vector<intensityType> referenceValue = { 102, 166, 169, 136, 155, 133, 207, 177, 119 };
  vector<intensityType> result = gaussianFilterSTDParallel(image, countRows, countColumns, countThreads);
  ASSERT_EQ(result, referenceValue);
}

TEST(STD_parallel, Works_with_4x3_image) {
...
TEST(STD_parallel, Works_with_full_black_image) {
  const int countRows = 3;
  const int countColumns = 3;
  const int countThreads = 4;
...
TEST(STD_parallel, Works_with_full_white_image) {
  const int countRows = 3;
  const int countColumns = 3;
  const int countThreads = 4;
...
TEST(STD_parallel, Can_calculate_150x177_image) {
