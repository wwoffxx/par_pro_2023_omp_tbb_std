...
#include "../../../modules/task_4/chernova_a_ccs_matrix_mult/ccs_matrix_mult.h"
...
const MatrixCSS parallelCCSMultiplication(const MatrixCSS& left,
  ...
  int dataPortion = tLeft.numberColumns / numberOfThread;
  ...
  for (int i = 0; i < numberOfThread; ++i) {
    start = i * dataPortion;
    end = i != numberOfThread - 1 ? (i + 1) * dataPortion : tLeft.numberColumns;

    threads[i] = std::thread(
        [&](const int start, const int end) {
          for (int leftColumn = start; leftColumn < end; ++leftColumn) {
