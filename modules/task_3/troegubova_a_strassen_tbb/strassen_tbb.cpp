...
#include <stdexcept>
#include <string>
#include "../../../modules/task_3/troegubova_a_strassen_tbb/strassen_tbb.h"
...
std::vector<double> multiplyStrassenTbb(const std::vector<double>& A, const std::vector<double>& B, bool parallel) {
    ...
    if (block_size == 1) {
        ...
        if (parallel) {
            tbb::task_group task;
            task.run([&] { D = multiplyStrassenTbb(sumMatrix(blockA[0], blockA[3]),
                sumMatrix(blockB[0], blockB[3]), false); });
            task.run([&] { D1 = multiplyStrassenTbb(diffMatrix(blockA[1], blockA[3]),
                    sumMatrix(blockB[2], blockB[3]), false); });
            task.run([&] { D2 = multiplyStrassenTbb(diffMatrix(blockA[2], blockA[0]),
                    sumMatrix(blockB[0], blockB[1]), false); });
            task.run([&] { H1 = multiplyStrassenTbb(sumMatrix(blockA[0], blockA[1]),
                    blockB[3], false); });
            task.run([&] { H2 = multiplyStrassenTbb(sumMatrix(blockA[2], blockA[3]),
                    blockB[0], false); });
            task.run([&] { V1 = multiplyStrassenTbb(blockA[3],
                    diffMatrix(blockB[2], blockB[0]), false); });
            task.run([&] { V2 = multiplyStrassenTbb(blockA[0],
                    diffMatrix(blockB[1], blockB[3]), false); });
            task.wait();
        } else {
            D = multiplyStrassenTbb(sumMatrix(blockA[0], blockA[3]),
                sumMatrix(blockB[0], blockB[3]), false);
            D1 = multiplyStrassenTbb(diffMatrix(blockA[1], blockA[3]),
                sumMatrix(blockB[2], blockB[3]), false);
            D2 = multiplyStrassenTbb(diffMatrix(blockA[2], blockA[0]),
                sumMatrix(blockB[0], blockB[1]), false);
            H1 = multiplyStrassenTbb(sumMatrix(blockA[0], blockA[1]),
                blockB[3], false);
            H2 = multiplyStrassenTbb(sumMatrix(blockA[2], blockA[3]),
                blockB[0], false);
            V1 = multiplyStrassenTbb(blockA[3],
                diffMatrix(blockB[2], blockB[0]), false);
            V2 = multiplyStrassenTbb(blockA[0],
                diffMatrix(blockB[1], blockB[3]), false);
        }
