...
#include "../../../modules/task_3/pikin_i_sparse_matrix_mult/sparse_matrix_mult.h"
...
CRSMatrixMultiplicator::CRSMatrixMultiplicator(const CRSMatrix& A_,
  const CRSMatrix& BT_,
  std::vector<std::vector<int>>* pColumns_,
  std::vector<std::vector<std::complex<double>>>* pValues_,
  std::vector<int>* pRowIndex_) : A(A_), BT(BT_), pColumns(pColumns_),
    pValues(pValues_), pRowIndex(pRowIndex_)
  {}

void CRSMatrixMultiplicator::operator()(const tbb::blocked_range<int>& range) const {
  int begin = range.begin();
  int end = range.end();
  int n = A.n;

  std::vector<std::vector<int>>& columns = *pColumns;
  std::vector<std::vector<std::complex<double>>>& values = *pValues;
  std::vector<int>& rowIndex = *pRowIndex;
...
CRSMatrix getProductOMP(const CRSMatrix& A, const CRSMatrix& B) {
  ...
  std::vector<int> rowIndex(n + 1ll, 0);
...
#pragma omp for private(j, k)
  ...
  std::vector<std::complex<double>> value(nz);
  std::vector<int> column(nz);

  std::vector<std::complex<double>>::iterator value_iter = value.begin();
  std::vector<int>::iterator column_iter = column.begin();
  for (i = 0; i < n; i++) {
    copy(values[i].begin(), values[i].end(), value_iter);
    value_iter += values[i].size();

    copy(columns[i].begin(), columns[i].end(), column_iter);
    column_iter += columns[i].size();
  }
...
CRSMatrix getProductTBB(const CRSMatrix& A, const CRSMatrix& B) {
  ...
  CRSMatrixMultiplicator(A, BT, &columns, &values, &rowIndex));
