...
TEST(TBB_version, Dense_int_matrices_3x3_product_test) {
  ...
  CRSMatrix prod = getProductTBB(crsA, crsB);

  ASSERT_EQ(crsC, prod);
}

TEST(TBB_version, Sparse_int_matrices_4x4_product_test) {
...
TEST(TBB_version, Sparse_complex_matrices_4x4_product_test) {
  std::vector<std::vector<cmp>> A = {
...
TEST(TBB_version, Dense_complex_matrices_4x4_product_test) {
  std::vector<std::vector<cmp>> A = {
...
TEST(TBB_version, Multiplication_by_zero_matrix_test) {
  std::vector<std::vector<cmp>> A = {
      {cmp(1, 0), cmp(2, -1.5), cmp(7, 0), cmp(9, 0)},
      {cmp(2, 0), cmp(4, 0), cmp(5.2, 4), cmp(10, 0)},
      {cmp(1.2, 0.8), cmp(5, 0), cmp(3.1, 0), cmp(11, 0)},
...
TEST(TBB_version, Comparison_of_Seq_and_TBB_version) {
  CRSMatrix A, B, C, D;
  ...
  // start_time = omp_get_wtime();
  // CRSMatrix E = getProductTBB(A, B);
  // end_time = omp_get_wtime();
  // search_time = end_time - start_time;
  // std::cout << "getProductTBB() time: " << search_time << " seconds\n";
  // ASSERT_EQ(C, D);
  // ASSERT_EQ(C, E);
  // }
