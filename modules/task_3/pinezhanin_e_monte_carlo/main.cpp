...
TEST(Monte_Carlo_tbb, Test_one_dimension) {
    ...
    double calc_value_integral_tbb = getIntegralMonteCarloTbb(f, a, b, number_points);
    double error = std::abs(calc_value_integral - calc_value_integral_tbb) / std::abs(calc_value_integral);

    ASSERT_LE(error, relative_error);
}

TEST(Monte_Carlo_tbb, Test_two_dimension) {
    const std::function<double(const std::vector<double>)> f =
...
TEST(Monte_Carlo_tbb, Test_three_dimension) {
    const std::function<double(const std::vector<double>)> f =
...
TEST(Monte_Carlo_tbb, Test_four_dimension) {
    const std::function<double(const std::vector<double>)> f =
...
TEST(Monte_Carlo_tbb, Test_five_dimension) {
    const std::function<double(const std::vector<double>)> f =
            [](const std::vector<double>& x) -> double {
