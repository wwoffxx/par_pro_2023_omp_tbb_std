...
#include "../../../modules/task_3/nikitin_a_strassen_tbb/strassen_tbb.h"
...
void get_p1(double* a11, double* a22, double* res1, double* b11, double* b22,
  ...
  tbb::parallel_for(tbb::blocked_range<int>(0, size * size),
                    [&](const tbb::blocked_range<int>& range) {
                      for (int i = range.begin(); i < range.end(); i++) {
                        res1[i] = a11[i] + a22[i];
                        res2[i] = b11[i] + b22[i];
...
void strassen(double* a, double* b, double* c, int size) {
  ...
  tbb::parallel_for(
      tbb::blocked_range<int>(0, half_split),
      [&](const tbb::blocked_range<int>& range) {
        for (int i = range.begin(); i < range.end(); i++) {
  ...
  tbb::parallel_for(tbb::blocked_range<int>(0, half_split * half_split),
                    [&](const tbb::blocked_range<int>& range) {
                      for (int i = range.begin(); i < range.end(); i++) {
      ...
      [&](const tbb::blocked_range<int>& range) {
        for (int i = range.begin(); i < range.end(); i++) {
          for (int j = 0; j < half_split; j++) {
            ...
            c[(half_split + i) * size + half_split + j] =
                c22[i * half_split + j];
          }
        }
      });
