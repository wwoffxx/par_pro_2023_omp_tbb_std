...
std::vector<std::vector<int>> build_convhull_img(
        const std::vector<std::vector<int>>& img, bool isPar) {
    int n, m;
    n = img.size();
    m = img[0].size();
    std::vector<std::vector<std::pair<int, int>>> compn =
            build_components(img, n, m, isPar);
    std::vector<std::vector<int>> res(n, std::vector<int>(m, 0));

    if (isPar) {
        parallel_for(tbb::blocked_range<size_t>(0, compn.size()),
            [&compn, &res, isPar](const tbb::blocked_range<size_t>& r){
                for (int k = static_cast<int>(r.begin());
                        k != static_cast<int>(r.end()); k++) {
                    compn[k] = graham(compn[k], isPar);
